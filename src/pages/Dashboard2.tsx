...
function HealthScoreComparison({ userScore }: { userScore: number }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/dashboard/health-scores")
      .then(res => res.json())
      .then(data => { if (isMounted) setScoreDistribution(data); })
      .catch(console.error);
  ...
  return (
    <Card className="shadow-card border-0">
      <CardHeader>
        ...
        <CardDescription>How your score compares to others</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="h-64">
        ...
        <div className="mt-4 text-center">
          <Badge variant={userScore >= 80 ? "default" : userScore >= 60 ? "secondary" : "destructive"}>
            Your Score: {userScore} ({userScore >= 70 ? "Above" : "Below"} Average)
          </Badge>
        </div>
      </CardContent>
    </Card>
...
function RiskFactorsAnalysis({ user }) {
  const [riskData, setRiskData] = useState([]);
  
  useEffect(() => {
  if (!user?.email) return;

  fetch(`/api/dashboard/risk-factors/${user.email}`)
    .then(res => res.json())
    .then(data => {
      const factors = [
        { name: "Smoking", userValue: data.userValues.smoking, populationAvg: data.populationAvg.smoking },
        { name: "Family History", userValue: data.userValues.familyHistory, populationAvg: data.populationAvg.familyHistory },
        { name: "Hypertension", userValue: data.userValues.hypertension, populationAvg: data.populationAvg.hypertension },
        { name: "Diabetes", userValue: data.userValues.diabetes, populationAvg: data.populationAvg.diabetes },
        { name: "Low Activity", userValue: data.userValues.lowActivity, populationAvg: data.populationAvg.lowActivity },
      ];
      setRiskData(factors);
    })
    .catch(console.error);
  ...
  return (
    <Card className="shadow-card border-0">
      <CardHeader>
        ...
        <CardDescription>Your risk factors vs. population prevalence</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={riskData} layout="vertical" margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Bar dataKey="populationAvg" fill="#8884d8" name="Population Avg" />
              <Bar dataKey="userValue" fill="#ff8042" name="You" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
...
function KeyMetricsTrend({ user }: { user: any }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/dashboard/lab-trends/${user.email}`)
      .then(res => res.json())
  ...
  return (
    <Card className="shadow-card border-0">
      <CardHeader>
        ...
        <CardDescription>Tracking your kidney health over time</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <YAxis />
              <Tooltip />
              <Line type="monotone" dataKey="eGFR" stroke="#8884d8" name="eGFR" />
              <Line type="monotone" dataKey="creatinine" stroke="#82ca9d" name="Creatinine" />
              <Line type="monotone" dataKey="systolicBP" stroke="#ff8042" name="Systolic BP" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
...
<HealthScoreComparison userScore={healthScore} />
<RiskFactorsAnalysis user={user} />
<KeyMetricsTrend user={user} />
